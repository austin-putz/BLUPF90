...
#====================================================================================================#
# blupf90.0.2.sh : Single and Two Trait Analysis
#====================================================================================================#

# This script is made to cycle through all combinations of 2 traits
...
if [ -z $1 ]; then        # check to see if any options are even given
	...
	printf "\nKeywords: \n" 1>&2
	printf "\t1) file.dat (datafile to replace)\n" 1>&2 
	printf "\t2) trait1 (single trait)\n" 1>&2
	printf "\t3) trait2 (in bivariate analyses)\n\n" 1>&2
 	...
 	while [[ $# -gt 0 ]]; do 
		...
		# set all the variable values
			...
			-bi)	shift
					...
					;;
			-st)	shift
					store=$1				# store every ___ samples
					;;
