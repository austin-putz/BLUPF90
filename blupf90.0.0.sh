...
if [ -z $1 ]; then        # check to see if any options are even given
	...
	printf "\t-ns    = number of samples" 1>&2
	printf "\t-bi    = burn-in" 1>&2
	printf "\t-st    = store every __ samples" 1>&2
...
if [[ $program == aireml ]] || [[ $program == thrgibbs1f90 ]]; then
